...
IplImage *getGray(const IplImage *img)
{
    ...
    if (!img) {
        error("Unable to create grayscale image.  No image supplied");
    }
...
void drawIpoints(IplImage *img, vector < Ipoint > &ipts, int tailSize)
{
    ...
    for (unsigned int i = 0; i < ipts.size(); i++) {
        ...
        if (tailSize) {
            cvLine(img, cvPoint(c1, r1), cvPoint(int (c1 + ipt->dx * tailSize), int (r1 + ipt->dy * tailSize)), cvScalar(255, 255, 255), 1);
        }
    }
}
...
void drawWindows(IplImage *img, vector < Ipoint > &ipts)
{
    ...
    for (unsigned int i = 0; i < ipts.size(); i++) {
        ...
        if (o) {                // Draw orientation line
            cvLine(img, cvPoint(x, y), cvPoint(fRound(s * cd + x), fRound(s * sd + y)), cvScalar(0, 255, 0), 1);
        } else {                // Green dot if using upright version
        ...
        cvLine(img, cvPoint(fRound(src[0].x), fRound(src[0].y)), cvPoint(fRound(src[1].x), fRound(src[1].y)), cvScalar(255, 0, 0), 2);
        cvLine(img, cvPoint(fRound(src[1].x), fRound(src[1].y)), cvPoint(fRound(src[2].x), fRound(src[2].y)), cvScalar(255, 0, 0), 2);
        cvLine(img, cvPoint(fRound(src[2].x), fRound(src[2].y)), cvPoint(fRound(src[3].x), fRound(src[3].y)), cvScalar(255, 0, 0), 2);
        cvLine(img, cvPoint(fRound(src[3].x), fRound(src[3].y)), cvPoint(fRound(src[0].x), fRound(src[0].y)), cvScalar(255, 0, 0), 2);
